...
{
	DamageFieldRender::DamageFieldRender()
	{
		m_Texture = 0;
	...
	void DamageFieldRender::LoadXml(const char* path)
	{
	...
	void DamageFieldRender::SetParentNode(CCNode* parent)
	{
	...
	void DamageFieldRender::SetPositionXY(int x, int y)
	{
		m_Sprite->setPosition(ccp(x-160,y-70));
	}
	void DamageFieldRender::SetPositionZ(int z)
	{
		m_Sprite->getParent()->reorderChild(m_Sprite,z);
	}
	void DamageFieldRender::SetVisible(bool value)
	{
		m_Sprite->setVisible(value);
	}
	void DamageFieldRender::InstantDestroy()
	{
