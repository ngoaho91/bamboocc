...
namespace FightEngine
{
	...
	void MissileRender::LoadXml(const char* path)
	{
		// skill/missile/47-/47-.xml
	...
	void MissileRender::SetParentNode(CCNode* parent)
	{
	...
	void MissileRender::SetPositionXY(int x, int y)
	{
		m_Sprite->setPosition(ccp(x-160,y-70));
	}
	void MissileRender::SetPositionZ(int z)
	{
		m_Sprite->getParent()->reorderChild(m_Sprite,z);
	}
	void MissileRender::SetDirection(int value)
	{
		m_Sprite->stopAllActions();
		m_Sprite->runAction(m_Actions[value]);
	}
	void MissileRender::SetVisible(bool value)
	{
		m_Sprite->setVisible(value);
	}
	void MissileRender::InstantDestroy()
	{
